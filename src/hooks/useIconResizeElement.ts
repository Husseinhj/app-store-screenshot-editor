...
interface UseIconResizeElementOptions {
  elementId: string;
  canvasWidth: number;
  canvasHeight: number;
  scale: number;
...
export function useIconResizeElement({ elementId, canvasWidth, canvasHeight, scale }: UseIconResizeElementOptions) {
    ...
    (handle: ResizeHandle, e: React.MouseEvent) => {
      ...
      const handleMouseMove = (ev: MouseEvent) => {
        ...
        rafRef.current = requestAnimationFrame(() => {
          useProjectStore.getState().updateAppIconElementTransform(elementId, update);
        });
      };
